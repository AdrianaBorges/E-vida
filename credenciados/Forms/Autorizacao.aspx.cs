...
namespace eVidaCredenciados.Forms {
	public partial class Autorizacao : FormPageBase {
		...
		protected override void PageLoad(object sender, EventArgs e) {
			if (!IsPostBack) {
                PRedeAtendimentoVO cred = UsuarioLogado.RedeAtendimento;
				...
				txtRazaoSocial.Text = cred.Nome;				
				txtCpfCnpj.Text = FormatUtil.FormatCpfCnpj(cred.Tippe, cred.Cpfcgc);
				...
				dpdUfConselho.DataSource = LocatorDataBO.Instance.ListarUf();
		...
		private void Bind(int id) {
            ...
            if (vo == null || vo.RedeAtendimento == null || vo.RedeAtendimento.Codigo.Trim() != UsuarioLogado.RedeAtendimento.Codigo.Trim())
            {
				this.ShowError("Identificador de requisição inválido!");
				return;
			}

			CanEdit = vo.Status == StatusAutorizacao.ENVIADA && vo.Origem == OrigemAutorizacao.CRED;
			...
			txtCartao.Text = vo.Usuario.Matant;
			txtDataSolicitacao.Text = vo.DataSolicitacao.ToString("dd/MM/yyyy HH:mm");
			txtStatus.Text = AutorizacaoTradutorHelper.TraduzStatus(vo.Status);
			txtDataAutorizacao.Text = vo.DataAutorizacao != null ? vo.DataAutorizacao.Value.ToString("dd/MM/yyyy HH:mm") : "";
            ...
            BindBeneficiario(vo.Usuario.Codint.Trim(), vo.Usuario.Codemp.Trim(), vo.Usuario.Matric.Trim(), vo.Usuario.Tipreg.Trim());

            txtCpfCnpj.Text = FormatUtil.FormatCpfCnpj(UsuarioLogado.RedeAtendimento.Tippe.Trim(), vo.RedeAtendimento.Cpfcgc.Trim());
			...
			if (vo.Hospital != null) {
				...
				lblHospitalMan.Visible = vo.Hospital.Codigo.Trim() == "";
			}
			...
			if (CanEdit) {
				...
				if (!CanAddFile) {
					...
					btnEnviarDoc.Visible = true;
				}
			}
		}

        private void BindBeneficiario(string codint, string codemp, string matric, string tipreg)
        {			
			PUsuarioVO beneficiario = PUsuarioBO.Instance.GetUsuario(codint, codemp, matric, tipreg);
			BindBeneficiario(beneficiario);
		}

		private void BindBeneficiario(string cartao) {
			PUsuarioVO beneficiario = PUsuarioBO.Instance.GetUsuarioByCartao(cartao);
			BindBeneficiario(beneficiario);
		}

		private void BindBeneficiario(PUsuarioVO beneficiario) {
			hidCodBeneficiario.Value = string.Empty;
			if (beneficiario == null) {
				this.ShowError("Beneficiário não encontrado!");
				txtNomeBenef.Text = string.Empty;
				txtPlano.Text = string.Empty;
				txtMunicipioUf.Text = string.Empty;
			} else {

                hidCodBeneficiario.Value = beneficiario.Codint.Trim() + "|" + beneficiario.Codemp.Trim() + "|" + beneficiario.Matric.Trim() + "|" + beneficiario.Tipreg.Trim();

				// nova autorizacao, filtrar apenas beneficiarios ativos
				if (string.IsNullOrEmpty(litProtocolo.Value)) {
                    if (beneficiario.Datblo != "        " && Int32.Parse(beneficiario.Datblo) <= Int32.Parse(DateTime.Today.ToString("yyyyMMdd")))
                    {
						this.ShowError("O beneficiário está INATIVO. Não é possível criar nova solicitação para ele.");
						return;
					}
				} else {
                ...
                PFamiliaProdutoVO benefPlano = PFamiliaBO.Instance.GetFamiliaProduto(beneficiario.Codint, beneficiario.Codemp, beneficiario.Matric, beneficiario.Tipreg);
				PProdutoSaudeVO plano = PLocatorDataBO.Instance.GetProdutoSaude(benefPlano.Codpla);

				txtNomeBenef.Text = beneficiario.Nomusr;
				txtPlano.Text = plano.Descri;
				txtMunicipioUf.Text = beneficiario.Munici + " / " + beneficiario.Estado;
			}
		}

		private void ChangeBeneficiario() {			
			txtNomeBenef.Text = string.Empty;
			txtPlano.Text = string.Empty;
			txtMunicipioUf.Text = string.Empty;
			BindBeneficiario(txtCartao.Text);
		}
		...
		private bool ValidateRequired() {
			...
			if (string.IsNullOrEmpty(txtCartao.Text)) {
				this.AddErrorMessage(lst, txtCartao, "Informe o cartão do beneficiário!");
			}

			if (string.IsNullOrEmpty(hidCodBeneficiario.Value)) {
				this.AddErrorMessage(lst, txtCartao, "Beneficiário inválido!");
			}
			...
			DataTable dtItens = RetrieveItemsFromDataList();
			if (dtItens == null || dtItens.Rows.Count == 0) {
				this.AddErrorMessage(lst, btnAdicionarItem, "Informe pelo menos um procedimento!");
			}
		...
		private string UrgenciaMessage {
			get {
				return "Atendimentos de Urgência/Emergência deverão ter pronto atendimento e posteriormente comunicados com até 02 (dois) dias úteis da ocorrência. Contato: autoriza@e-vida.org.br ou 0800-607-8300";
			}
		}
		...
		private void Salvar() {
			...
			if (!string.IsNullOrEmpty(litProtocolo.Value)) {
				...
				vo.RedeAtendimento = UsuarioLogado.RedeAtendimento;
                vo.Hospital = UsuarioLogado.RedeAtendimento;
			}

			string[] dados_beneficiario = hidCodBeneficiario.Value.Split('|');
			...
			vo.Obs = txtObservacao.Text;
			List<AutorizacaoProcedimentoVO> lstProcs = Table2Items(dtItems); 
            ...
            AutorizacaoBO.Instance.Salvar(vo, lstProcs, lstArqs, UsuarioLogado.RedeAtendimento);
		...
		private void EnviarDocumentos() {
			...
			AutorizacaoBO.Instance.EnviarDocumentos(OrigemAutorizacao.CRED, vo, null, lstArqs);
		...
		protected void dpdCarater_SelectedIndexChanged(object sender, EventArgs e) {
			if (dpdCarater.SelectedValue.Equals("URGENCIA")) {
				this.ShowError(UrgenciaMessage);
			}
		}
