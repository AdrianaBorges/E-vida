<%@ Page Title="SOLICITAÇÃO DE AUTORIZAÇÃO" Language="C#" MasterPageFile="~/Internal/Internal.Master" AutoEventWireup="true" CodeBehind="BuscaAutorizacao.aspx.cs" Inherits="eVidaBeneficiarios.Forms.BuscaAutorizacao" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        ...
        function dlgOpen(handler, ev, ui) {
            var src = "";
            switch (handler.tipo) {
                case POP_REVALIDAR: src = 'PopAutorizacaoRevalidar.aspx?'; break;
            }
            src += '&ID=' + handler.id;
            setLocatorUrl(src);
        }
        ...
        function openRevalidar(obj, id, row) {
            //if (confirm("Deseja realmente solicitar a revalidação da solicitação?")) {
    	...
    	function openManual() {
    		open('../docs/MANUAL_BENEFICIARIO_AUTORIZACAO.pdf?DT=' + (new Date()).getDate(), 'manual');
    	}
    	function openRelacaoAuditor() {
    	    open('../docs/PERICIA_REGIONAL.pdf?DT=' + (new Date()).getDate(), 'pericia');
    	}
...
<asp:Content ID="Content2" ContentPlaceHolderID="placeHolder" runat="server">
	...
	<tr style="height:300px">
		<td colspan="4">
			<asp:Button ID="btnRelacaoAuditores" runat="server" Text="Relação de Auditores" OnClientClick="return openRelacaoAuditor()" />
			<asp:Button ID="btnManual" runat="server" Text="Manual de Uso" OnClientClick="return openManual()" />
			...
			<asp:GridView ID="gdvRelatorio" runat="server" AutoGenerateColumns="false" DataKeyNames="cd_autorizacao"
				AllowSorting="false" CssClass="tabela" Width="1300px" OnRowDataBound="gdvRelatorio_RowDataBound">
                <Columns>
					...
					<asp:BoundField HeaderText="Plano" DataField="plano_DESCRI" Visible="false" />
